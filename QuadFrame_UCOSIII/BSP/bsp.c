...
#define  BSP_GPIOC_LED1                        DEF_BIT_13
#define  BSP_GPIOC_LED2                        DEF_BIT_14
...
*               (2) Init the interrupt
										...
										Init the SPI1,LED,and Debug_pin
*********************************************************************************************************
*/
...
void  BSP_Init (void)
{
    ...
    BSP_LED_Init();                                             /* Initialize user LEDs                                 */
		...
		/* Init PB8 PB9 for Logic Analysis */
    BSP_PeriphEn(BSP_PERIPH_ID_GPIOB);
    gpio_init.GPIO_Pin   = LOGIC_DBG_PIN1|LOGIC_DBG_PIN2; /* attached to PWM7, PWM8 pin */
    gpio_init.GPIO_Mode  = GPIO_Mode_OUT;
    gpio_init.GPIO_OType = GPIO_OType_PP;
    gpio_init.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    gpio_init.GPIO_Speed = GPIO_Speed_100MHz;
    GPIO_Init(LOGIC_DBG_PORT, &gpio_init);
	  LOGIC_DBG_PIN1_L;
	  LOGIC_DBG_PIN2_L;
...
static void  BSP_LED_Init()
{
    ...
    gpio_init.GPIO_Pin   = BSP_GPIOC_LED1;
    gpio_init.GPIO_Mode  = GPIO_Mode_OUT;
    gpio_init.GPIO_OType = GPIO_OType_PP;
    gpio_init.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    ...
    BSP_PeriphEn(BSP_PERIPH_ID_GPIOC);
    gpio_init.GPIO_Pin   = BSP_GPIOC_LED2;
    gpio_init.GPIO_Mode  = GPIO_Mode_OUT;
    gpio_init.GPIO_OType = GPIO_OType_PP;
    gpio_init.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    gpio_init.GPIO_Speed = GPIO_Speed_50MHz;
...
void  BSP_LED_Toggle (CPU_INT08U  led)
{
    ...
    switch (led) {
        case 0u:
             pins  =  GPIO_ReadOutputData(GPIOC);               /* Read data (current state of output pins) in GPIOA    */
             pins ^=  BSP_GPIOC_LED1;                           /* Tells which pins (other than pin for LED1) are ON    */
             GPIO_SetBits(  GPIOC,   pins  &  BSP_GPIOC_LED1);  /* If there are any,turn them OFF and leave LED1 ON     */
