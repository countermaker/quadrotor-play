...
/* ----------------- APPLICATION GLOBALS -------------- */
static  OS_TCB   AppTaskStartTCB;                                   //task tcb
__align(8)  CPU_STK  AppTaskStartStk[APP_CFG_TASK_START_STK_SIZE];  //stack
...
int main(void)
{
  ...
  CPU_Init();                                                 /* Initialize the uC/CPU Services(CPU_32 TIMER)         */
  Mem_Init();                                                 /* Initialize Memory Management Module                  */
  Math_Init();                                                /* Initialize Mathematical Module                      	*/

  OSInit(&err);                                               /* Init uC/OS-III(Create five task).                    */
...
static  void  AppTaskStart (void *p_arg)
{
  ...
  App_OS_SetAllHooks();                                       /*hook function initialize*/
  ...
  CPU_IntDisMeasMaxCurReset();
	...
	BSP_PWM2_Init(TIM4_PWM_HIGHEST);                         		/*stacker*/
...
static  void  AppObjCreate (void)
{
  uint8_t status;
	...
	status = BSP_OS_SemCreate(&Sem_SerPrintUpdate, 0, "Sem_SerPrintUpdate");		/*signal*/
	if (status == DEF_FAIL){
