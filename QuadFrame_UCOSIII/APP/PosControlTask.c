...
*
* Description : 
								Wait for the post from Attitude_Task
								Get the attitude.
								Get the position,velocity,target_position target_acc.
								Get the des_acc
*								Calculate the des_euler
								Send post to Euler_control
* Argument(s) : none
*
* Return(s)   : none.
*
...
void PosControlTask(void *p_arg)
{
  ...
  while (1)
  {
    ...
    if (g_FlagPosHoldXY == SET) {
      /* X */
      error_pos.x   = DLPF(Constraint_f(target_pos.x - pos_ref.x, -0.3, 0.3), error_pos.x , 5.0f * HZ2RAD, dt);/*5hz*/
      error_vel.x = target_vel.x - 1.2f*vel_ref.x;
      i_error_pos.x = Constraint_f(i_error_pos.x + error_pos.x * dt, -2, 2);

      /* Y */
      error_pos.y   = DLPF(Constraint_f(target_pos.y - pos_ref.y, -0.3, 0.3), error_pos.y , 5.0f * HZ2RAD, dt);/*5hz*/
      ...
      i_error_pos.x = i_error_pos.y = 0;
		...
		g_TestTmpData1 = euler_att.yaw;
 		g_TestTmpData2 = euler_pos.pitch;
 		g_TestTmpData3 = euler_pos.roll;
